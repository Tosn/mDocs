...
> ⏳ **待你在本机手动执行**——需真实模型 Key 与 Electron 窗口交互，无法自动化。
> 逐项清单见 **`feature/acceptance.md`**；自动化测试（202 个）已全绿，覆盖各模块逻辑。

- ⬜ R1. 按 `spec.md` 第 5 章逐条用户故事验收（A1–A4 / B1–B3 / C1–C2 / D1 / E1–E6）。→ acceptance.md「R1」
- ⬜ R2. 按 `spec.md` 第 6 章逐条边界情况核对（含无网降级、回收站启动清理、网页爬取失败、**@范围失效、登录爬取中断**）。→ acceptance.md「R2」
- ⬜ R3. 手动验收：@文件/@文件夹 限定问答范围；需登录网页的内置窗口登录后抓取当前页。→ acceptance.md「R3」
